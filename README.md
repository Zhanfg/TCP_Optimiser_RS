...
- 总览、遥测、策略、日志四个主要页面；总览直接展示当前策略栈与运行内核的 KO bundle 匹配状态。
- 实时显示吞吐量、重传率、拥塞窗口和 RTT 图表。
- 通过 Rust 单次 JSON 快照读取运行状态与统计数据，并逐项核验已保存策略、内核实际值和接口 qdisc；发现偏差时可安全重应用策略，不主动断开现有连接。
...
- 完整显示已知算法和 qdisc，并区分**运行时已有能力**与**当前内核匹配、可按需加载的内置 KO 能力**；没有精确版本/KMI 匹配时不会强制加载模块。
- 支持 `fq`、`fq_codel`、`cake`、`pfifo_fast`、`codel`、`fq_pie`、`pfifo`、`pie`、`pfifo_head_drop` 九种队列调度器；应用后回读验证，并在网络恢复或内核重置队列时自动修复。
- 提供五组内置预设，并支持通过 JSON 导入、导出自定义预设。
- 提供 39 项运行时检测的高级内核参数，覆盖连接生命周期、缓冲区、队列、丢包恢复、PLB、低延迟轮询和 conntrack；内核未提供的节点仅显示为不可用，不会写入。
